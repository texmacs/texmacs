...
(texmacs-module (utils library list))
...
(tm-define (list-partition l pred?)
  ...
  (:argument l "list to be partitioned")
...
(tm-property (list-break l pred?)
  ;; defined in (boot srfi)
  (:type (forall T (-> (list T) (-> T bool) (cross (list T) (list T)))))
  (:synopsis "Break @l at the first element satisfying @pred?.")
  (:argument l "list to be broken")
  (:argument pred? "predicate")
...
(tm-define (list-scatter l pred? keep?)
  ...
  (:argument l "list to be broken")
  (:argument pred? "predicate")
  (:argument keep? "keep break points?")
