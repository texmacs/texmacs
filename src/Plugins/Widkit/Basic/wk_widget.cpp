...
wk_widget_rep::set_blackbox (string key, blackbox val) {
  ...
  else if (type_id == type_helper<pair<SI,SI> >::id) {
    pair<SI,SI > p= open_box<pair<SI,SI> > (val);
    wk_widget (this) << ::set_coord2 (key, p.x1, p.x2);
  }
  else if (type_id == type_helper<quadruple<SI,SI,SI,SI> >::id) {
    quadruple<SI,SI,SI,SI > q= open_box<quadruple<SI,SI,SI,SI> > (val);
    wk_widget (this) << ::set_coord4 (key, q.x1, q.x2, q.x3, q.x4);
  }
...
wk_widget_rep::get_blackbox (string key, int type_id) {
  ...
  if (type_id == type_helper<int>::id) {
    ...
    SI c1, c2;
    wk_widget (this) << ::get_coord2 (key, c1, c2);
    //cout << "Got " << key << " =  " << coord2 (c1, c2) << "\n";
    return close_box<coord2> (coord2 (c1, c2));
  }
  else if (type_id == type_helper<quadruple<SI,SI,SI,SI> >::id) {
    typedef quadruple<SI,SI,SI,SI> coord4;
    SI c1, c2, c3, c4;
    wk_widget (this) << ::get_coord4 (key, c1, c2, c3, c4);
    //cout << "Got " << key << " = " << coord4 (c1, c2, c3, c4) << "\n";
