...
mingw_system (array<string> arg,
  ...
  const int rd_size=n_out?ch[n_in].sz:0;
  array<char> buffer (rd_size);
  ...
  do {
    busy= false;
    if (texmacs_time () - last_transfer_time > 100) _sleep (100);
    if (texmacs_time () - last_transfer_time > 5000) { // FIXME?
      last_transfer_time= texmacs_time ();
      _unix_system_warn (process.getpid(), which, "silent spawn process");
    }
	for (int i= 0; i < n_in; i++) {
		if (N(str_in[i]) > pos_in[i]) {
			int m= min (ch[i].sz/2, N(str_in[i]) - pos_in[i]);  // do not fill the pipe
	...
	for (int i= 0; i < n_out; i++) {
		int m= ch[i+n_in].eofread (A (buffer), rd_size);
		if (m >= 0) {
			if (m > 0) {
				(*(str_out[i])) << string ((const char*) A (buffer), m);
				last_transfer_time= texmacs_time ();
				busy=true;
				debug_io << "Read Str:" << (*(str_out[i])) << "#end\n";
			}
		} else ch[i+n_in].close();
	}
  } while (busy);
  // wait for process
