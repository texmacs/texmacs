...
#if (QT_VERSION >= 0x050000)
   #define PXM_SIZE (pxm->size()/pxm->devicePixelRatio())
#else
   #define PXM_SIZE (pxm->size())
#endif
    QImage *pxm = xpm_image ("tm_new.xpm");
    QSize sz = (pxm ? PXM_SIZE : QSize (24, 24));
    tweak_iconbar_size (sz);
    mainToolBar->setIconSize (sz);
    pxm = xpm_image ("tm_section.xpm");
    sz = (pxm ? PXM_SIZE : QSize (20, 20));
    tweak_iconbar_size (sz);
    modeToolBar->setIconSize (sz);
    pxm = xpm_image ("tm_add.xpm");
    sz = (pxm ? PXM_SIZE : QSize (16, 16));
    tweak_iconbar_size (sz);
