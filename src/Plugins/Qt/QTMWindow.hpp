...
public:
  ...
  : QWidget (parent) { 
    if (DEBUG_QT) debug_qt << "Create QTMPlainWindow" << LF;
  }
  virtual ~QTMPlainWindow () {
    if (DEBUG_QT) debug_qt << "Delete QTMPlainWindow" << LF;
  }
...
public:
  ...
  : QMainWindow (parent) { 
    if (DEBUG_QT) debug_qt << "Create QTMWindow" << LF;
  }
  virtual ~QTMWindow () {
    if (DEBUG_QT) debug_qt << "Delete QTMWindow" << LF;
  }
