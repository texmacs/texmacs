		     ...
		     SI& y1, SI& y2, SI& scy)
{
  ...
  if (N(attrs) == 9 && is_atomic (attrs[8]) &&
      (starts (attrs[8]->label, "n") || starts (attrs[8]->label, "s"))) {
    SI d= env->as_length (attrs[6]) + env->as_length (attrs[7]);
    y2= max (y1, y2-d);
  }
...
make_hor_bar (edit_env env, path ip, SI x1, SI x2, SI h, SI border,
	      SI X1, SI X2)
{
  box mask1= empty_box (ip, x1+border, border, x2-border, h-border);
  box mask2= empty_box (ip, X1+border, 2*border, X2-border, h-2*border);
...
make_ver_bar (edit_env env, path ip, SI y1, SI y2, SI w, SI border,
	      SI Y1, SI Y2)
{
  box mask1= empty_box (ip, border, y1+border, w-border, y2-border);
  box mask2= empty_box (ip, 2*border, Y1+border, w-2*border, Y2-border);
  box hl1  = highlight_box (ip, mask1, border, 0, 0, env->dis->light_grey,
			    env->dis->grey, env->dis->white);
  box hl2  = highlight_box (ip, mask2, border, 0, 0, env->dis->light_grey,
			    env->dis->white, env->dis->grey);
		 ...
		 box inner, tree xt, tree yt, SI scx, SI scy)
{
  ...
  xs[0]= (ver < 0? w+pad: 0);
  ys[0]= (hor < 0? w+pad: 0);
  SI  border= env->get_int (SFACTOR) * PIXEL;
  if (hor != 0 && inner->w() > b->w()) {
    ...
    box hor_bar= make_hor_bar (env, dip, b->x1, b->x2, w, border, X1, X2);
    hor_bar= scrollbar_box (dip, hor_bar, false, X2-X1, xt);
    bs << hor_bar;
    xs << (ver < 0? w+pad: 0);
    ys << (hor < 0? b->y1: b->y2+pad);
  }
  if (ver != 0 && inner->h() > b->h()) {
    ...
    box ver_bar= make_ver_bar (env, dip, b->y1, b->y2, w, border, Y1, Y2);
    ver_bar= scrollbar_box (dip, ver_bar, true, Y2-Y1, yt);
    bs << ver_bar;
    xs << (ver < 0? b->x1: b->x2+pad);
    ys << (hor < 0? w+pad: 0);
  }
  return composite_box (ip, bs, xs, ys);
}
