...
static void
add_frames (array<box>& v, path ip, url u, int w, int h, int px,
            int a, int msecs) {
  if (is_none (u)) return;
  else if (is_or (u)) {
    add_frames (v, ip, u[1], w, h, px, a, msecs);
    add_frames (v, ip, u[2], w, h, px, a, msecs);
  }
  else {
    picture pic= lazy_picture (u, w / px, h / px);
    box imb= image_box (ip, pic, w, h, a);
...
box
video_box (path ip, url u, SI w, SI h, SI pixel,
           int alpha, int msecs, bool repeated) {
  url frames= decompose_gif (u);
  if (is_none (frames)) return empty_box (ip, 0, 0, w, h);
  array<box> bs;
  add_frames (bs, decorate (ip), frames, w, h, pixel, alpha, msecs);
