...
qt_chooser_widget_rep::query (slot s, int type_id) {
  ...
  case SLOT_POSITION:  
	...
	fatal_error ("type mismatch (SLOT_POSITION)", "qt_chooser_widget_rep::query");
  ...
  case SLOT_SIZE:
    {
      typedef pair<SI,SI> coord2;
      if (type_id != type_helper<coord2>::id)
	fatal_error ("type mismatch (SLOT_SIZE)", "qt_chooser_widget_rep::query");
...
widget qt_chooser_widget_rep::plain_window_widget (string s)
{
  win_title = s;
  return this;
}
...
public:
  qt_field_widget_rep(qt_input_widget_rep *_parent) : widget_rep(), prompt(""), input(""),  proposals(), parent(_parent) {};
  virtual void send (slot s, blackbox val);
  virtual blackbox query (slot s, int type_id);
...
qt_field_widget_rep::query (slot s, int type_id) {
  ...
  case SLOT_STRING_INPUT:
    {
      if (type_id != type_helper<string>::id)
	fatal_error ("type mismatch (SLOT_STRING_INPUT)", "qt_field_widget_rep::query");
  ...
  case SLOT_POSITION:  
    {
      typedef pair<SI,SI> coord2;
      if (type_id != type_helper<coord2>::id)
	fatal_error ("type mismatch (SLOT_POSITION)", "qt_input_widget_rep::query");
      return close_box<coord2> (position);
    }
  case SLOT_SIZE:
    {
      typedef pair<SI,SI> coord2;
      if (type_id != type_helper<coord2>::id)
	fatal_error ("type mismatch (SLOT_SIZE)", "qt_input_widget_rep::query");
      return close_box<coord2> (size);
    }
  case SLOT_STRING_INPUT:
...
void qt_input_widget_rep::perform_dialog()
{
  ...
  for(int i=0; i<N(fields); i++) {
    ...
    QString item = QInputDialog::getItem(NULL, "Interactive Prompt", label, items, 0, true, &ok );
    if (ok && !item.isEmpty())
      {
...
void qt_tm_widget_rep::do_interactive_prompt()
{
  ...
  if (ok && !item.isEmpty())
    {
      ((qt_input_text_widget_rep*)int_input.rep)->text = scm_quote(from_qstring(item));
