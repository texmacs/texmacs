...
INTERFACE INLINE int is_string(cell_ptr p)     { return (type(p)==T_STRING); }
#define strvalue(p)      ((p)->_object._string._svalue)
#define strlength(p)        ((p)->_object._string._length)

INTERFACE static int is_list(scheme *sc, cell_ptr p);
INTERFACE INLINE int is_vector(cell_ptr p)    { return (type(p)==T_VECTOR); }
INTERFACE static void fill_vector(cell_ptr vec, cell_ptr obj);
INTERFACE static cell_ptr vector_elem(cell_ptr vec, int ielem);
INTERFACE static cell_ptr set_vector_elem(cell_ptr vec, int ielem, cell_ptr a);
INTERFACE INLINE int is_number(cell_ptr p)    { return (type(p)==T_NUMBER); }
...
INTERFACE INLINE int is_character(cell_ptr p) { return (type(p)==T_CHARACTER); }
INTERFACE INLINE char *string_value(cell_ptr p) { return strvalue(p); }
INTERFACE INLINE int string_length(cell_ptr p) { return strlength(p); }
INLINE num nvalue(cell_ptr p)       { return ((p)->_object._number); }
INTERFACE long ivalue(cell_ptr p)      { return (num_is_integer(p)?(p)->_object._number.value.ivalue:(long)(p)->_object._number.value.rvalue); }
INTERFACE double rvalue(cell_ptr p)    { return (!num_is_integer(p)?(p)->_object._number.value.rvalue:(double)(p)->_object._number.value.ivalue); }
...
INTERFACE  long charvalue(cell_ptr p)  { return ivalue_unchecked(p); }

INTERFACE INLINE int is_port(cell_ptr p)     { return (type(p)==T_PORT); }
INTERFACE INLINE int is_inport(cell_ptr p)  { return is_port(p) && p->_object._port->kind & port_input; }
INTERFACE INLINE int is_outport(cell_ptr p) { return is_port(p) && p->_object._port->kind & port_output; }

INTERFACE INLINE int is_pair(cell_ptr p)     { return (type(p)==T_PAIR); }
INTERFACE INLINE int is_blackbox(cell_ptr p)     { return (type(p)==T_BLACKBOX); }

#define car(p)           ((p)->_object._cons._car)
#define cdr(p)           ((p)->_object._cons._cdr)
INTERFACE cell_ptr pair_car(cell_ptr p)   { return car(p); }
INTERFACE cell_ptr pair_cdr(cell_ptr p)   { return cdr(p); }
INTERFACE cell_ptr set_car(cell_ptr p, cell_ptr q) { return car(p)=q; }
INTERFACE cell_ptr set_cdr(cell_ptr p, cell_ptr q) { return cdr(p)=q; }

INTERFACE INLINE void *blackboxvalue(cell_ptr p)  { return car(p); }

INTERFACE INLINE int is_symbol(cell_ptr p)   { return (type(p)==T_SYMBOL); }
INTERFACE INLINE char *symname(cell_ptr p)   { return strvalue(car(p)); }
INTERFACE INLINE int symlen(cell_ptr p)   { return strlength(car(p)); }
#if USE_PLIST
SCHEME_EXPORT INLINE int hasprop(cell_ptr p)     { return (typeflag(p)&T_SYMBOL); }
#define symprop(p)       cdr(p)
#endif

INTERFACE INLINE int is_syntax(cell_ptr p)   { return (typeflag(p)&T_SYNTAX); }
INTERFACE INLINE int is_proc(cell_ptr p)     { return (type(p)==T_PROC); }
INTERFACE INLINE int is_foreign(cell_ptr p)  { return (type(p)==T_FOREIGN); }
INTERFACE INLINE char *syntaxname(cell_ptr p) { return strvalue(car(p)); }
#define procnum(p)       ivalue(p)
static const char *procname(cell_ptr x);

INTERFACE INLINE int is_closure(cell_ptr p)  { return (type(p)==T_CLOSURE); }
INTERFACE INLINE int is_macro(cell_ptr p)    { return (type(p)==T_MACRO); }
INTERFACE INLINE cell_ptr closure_code(cell_ptr p)   { return car(p); }
INTERFACE INLINE cell_ptr closure_env(cell_ptr p)    { return cdr(p); }

INTERFACE INLINE int is_continuation(cell_ptr p)    { return (type(p)==T_CONTINUATION); }
#define cont_dump(p)     cdr(p)

/* To do: promise should be forced ONCE only */
INTERFACE INLINE int is_promise(cell_ptr p)  { return (type(p)==T_PROMISE); }

INTERFACE INLINE int is_environment(cell_ptr p) { return (type(p)==T_ENVIRONMENT); }
...
INTERFACE INLINE int is_immutable(cell_ptr p) { return (typeflag(p)&T_IMMUTABLE); }
/*#define setimmutable(p)  typeflag(p) |= T_IMMUTABLE*/
INTERFACE INLINE void setimmutable(cell_ptr p) { typeflag(p) |= T_IMMUTABLE; }
...
{
  cell_ptr holder = get_cell_x(sc, recent, extra);
...
static cell_ptr get_cell(scheme *sc, cell_ptr a, cell_ptr b)
{
...
#if defined TSGRIND
static void check_cell_alloced(cell_ptr p, int expect_alloced)
{
...
static void check_range_alloced(cell_ptr p, int n, int expect_alloced)
{
  int i;
  for(i = 0;i<n;i++)
...
INTERFACE static cell_ptr mk_vector(scheme *sc, int len)
...
static cell_ptr port_from_file(scheme *sc, FILE *f, int prop) {
  port *pt;
  pt=port_rep_from_file(sc,f,prop);
  if(pt==0) {
    return sc->NIL;
...
static INLINE void new_slot_spec_in_env(scheme *sc, cell_ptr env,
                                        cell_ptr variable, cell_ptr value)
{
...
static cell_ptr find_slot_in_env(scheme *sc, cell_ptr env, cell_ptr hdl, int all)
{
  cell_ptr x,y;
  int location;
...
static INLINE void new_frame_in_env(scheme *sc, cell_ptr old_env)
{
...
static int is_list(scheme *sc, cell_ptr a)
...
static int is_any(cell_ptr p) { return 1;}
...
#if USE_INTERFACE
INTERFACE static cell_ptr s_cons(scheme *sc, cell_ptr a, cell_ptr b) {
 return cons(sc,a,b);
}
INTERFACE static cell_ptr s_immutable_cons(scheme *sc, cell_ptr a, cell_ptr b) {
...
cell_ptr scheme_apply0(scheme *sc, const char *procname)
...
cell_ptr scheme_call(scheme *sc, cell_ptr func, cell_ptr args)
{
...
cell_ptr scheme_eval(scheme *sc, cell_ptr obj)
{
  int old_repl = sc->interactive_repl;
  sc->interactive_repl = 0;
  save_from_C_call(sc);
