...
initialize_glue_server () {
  scm_install_procedure ("insert-kbd-wildcard",  tmg_insert_kbd_wildcard, 5, 0, 0);
  scm_install_procedure ("set-variant-keys",  tmg_set_variant_keys, 2, 0, 0);
  scm_install_procedure ("kbd-pre-rewrite",  tmg_kbd_pre_rewrite, 1, 0, 0);
  scm_install_procedure ("kbd-post-rewrite",  tmg_kbd_post_rewrite, 2, 0, 0);
  scm_install_procedure ("kbd-system-rewrite",  tmg_kbd_system_rewrite, 1, 0, 0);
  scm_install_procedure ("set-font-rules",  tmg_set_font_rules, 1, 0, 0);
  scm_install_procedure ("window-get-id",  tmg_window_get_id, 0, 0, 0);
  scm_install_procedure ("window-set-property",  tmg_window_set_property, 2, 0, 0);
  scm_install_procedure ("window-get-property",  tmg_window_get_property, 1, 0, 0);
  scm_install_procedure ("show-header",  tmg_show_header, 1, 0, 0);
  scm_install_procedure ("show-icon-bar",  tmg_show_icon_bar, 2, 0, 0);
  scm_install_procedure ("show-side-tools",  tmg_show_side_tools, 2, 0, 0);
  scm_install_procedure ("show-footer",  tmg_show_footer, 1, 0, 0);
  scm_install_procedure ("visible-header?",  tmg_visible_headerP, 0, 0, 0);
  scm_install_procedure ("visible-icon-bar?",  tmg_visible_icon_barP, 1, 0, 0);
  scm_install_procedure ("visible-side-tools?",  tmg_visible_side_toolsP, 1, 0, 0);
  scm_install_procedure ("visible-footer?",  tmg_visible_footerP, 0, 0, 0);
  scm_install_procedure ("full-screen-mode",  tmg_full_screen_mode, 2, 0, 0);
  scm_install_procedure ("full-screen?",  tmg_full_screenP, 0, 0, 0);
  scm_install_procedure ("full-screen-edit?",  tmg_full_screen_editP, 0, 0, 0);
  scm_install_procedure ("set-shrinking-factor",  tmg_set_shrinking_factor, 1, 0, 0);
  scm_install_procedure ("get-shrinking-factor",  tmg_get_shrinking_factor, 0, 0, 0);
  scm_install_procedure ("shell",  tmg_shell, 1, 0, 0);
  scm_install_procedure ("dialogue-end",  tmg_dialogue_end, 0, 0, 0);
  scm_install_procedure ("choose-file",  tmg_choose_file, 3, 0, 0);
  scm_install_procedure ("tm-interactive",  tmg_tm_interactive, 2, 0, 0);
  scm_install_procedure ("has-view?",  tmg_has_viewP, 0, 0, 0);
  scm_install_procedure ("buffer-unsaved?",  tmg_buffer_unsavedP, 0, 0, 0);
  scm_install_procedure ("exists-unsaved-buffer?",  tmg_exists_unsaved_bufferP, 0, 0, 0);
  scm_install_procedure ("pretend-save-buffer",  tmg_pretend_save_buffer, 0, 0, 0);
  scm_install_procedure ("set-name-buffer",  tmg_set_name_buffer, 1, 0, 0);
  scm_install_procedure ("get-name-buffer",  tmg_get_name_buffer, 0, 0, 0);
  scm_install_procedure ("get-name-buffer-path",  tmg_get_name_buffer_path, 1, 0, 0);
  scm_install_procedure ("set-abbr-buffer",  tmg_set_abbr_buffer, 1, 0, 0);
  scm_install_procedure ("get-abbr-buffer",  tmg_get_abbr_buffer, 0, 0, 0);
  scm_install_procedure ("new-buffer",  tmg_new_buffer, 0, 0, 0);
  scm_install_procedure ("switch-to-buffer-path",  tmg_switch_to_buffer_path, 1, 0, 0);
  scm_install_procedure ("switch-to-buffer",  tmg_switch_to_buffer, 1, 0, 0);
  scm_install_procedure ("switch-to-active-buffer",  tmg_switch_to_active_buffer, 1, 0, 0);
  scm_install_procedure ("revert-buffer",  tmg_revert_buffer, 0, 0, 0);
  scm_install_procedure ("kill-buffer",  tmg_kill_buffer, 0, 0, 0);
  scm_install_procedure ("open-buffer-in-window",  tmg_open_buffer_in_window, 3, 0, 0);
  scm_install_procedure ("open-window",  tmg_open_window, 0, 0, 0);
  scm_install_procedure ("open-window-geometry",  tmg_open_window_geometry, 1, 0, 0);
  scm_install_procedure ("clone-window",  tmg_clone_window, 0, 0, 0);
  scm_install_procedure ("kill-window",  tmg_kill_window, 0, 0, 0);
  scm_install_procedure ("kill-window-and-buffer",  tmg_kill_window_and_buffer, 0, 0, 0);
  scm_install_procedure ("no-name?",  tmg_no_nameP, 0, 0, 0);
  scm_install_procedure ("help-buffer?",  tmg_help_bufferP, 0, 0, 0);
  scm_install_procedure ("set-buffer",  tmg_set_buffer, 2, 0, 0);
  scm_install_procedure ("set-aux",  tmg_set_aux, 2, 0, 0);
  scm_install_procedure ("set-aux-buffer",  tmg_set_aux_buffer, 3, 0, 0);
  scm_install_procedure ("set-help-buffer",  tmg_set_help_buffer, 2, 0, 0);
  scm_install_procedure ("set-buffer-tree",  tmg_set_buffer_tree, 2, 0, 0);
  scm_install_procedure ("get-buffer-tree",  tmg_get_buffer_tree, 1, 0, 0);
  scm_install_procedure ("get-all-buffers",  tmg_get_all_buffers, 0, 0, 0);
  scm_install_procedure ("get-buffer-menu",  tmg_get_buffer_menu, 0, 0, 0);
  scm_install_procedure ("buffer-in-menu",  tmg_buffer_in_menu, 2, 0, 0);
  scm_install_procedure ("project-attach",  tmg_project_attach, 1, 0, 0);
  scm_install_procedure ("project-detach",  tmg_project_detach, 0, 0, 0);
  scm_install_procedure ("project-attached?",  tmg_project_attachedP, 0, 0, 0);
  scm_install_procedure ("get-project-buffer-menu",  tmg_get_project_buffer_menu, 0, 0, 0);
  scm_install_procedure ("window-current",  tmg_window_current, 0, 0, 0);
  scm_install_procedure ("window-list",  tmg_window_list, 0, 0, 0);
  scm_install_procedure ("buffer->windows",  tmg_buffer_2windows, 1, 0, 0);
  scm_install_procedure ("window->buffer",  tmg_window_2buffer, 1, 0, 0);
  scm_install_procedure ("window-set-buffer",  tmg_window_set_buffer, 2, 0, 0);
  scm_install_procedure ("window-focus",  tmg_window_focus, 1, 0, 0);
  scm_install_procedure ("texmacs-load-tree",  tmg_texmacs_load_tree, 2, 0, 0);
  scm_install_procedure ("texmacs-load-buffer",  tmg_texmacs_load_buffer, 4, 0, 0);
  scm_install_procedure ("texmacs-save-buffer",  tmg_texmacs_save_buffer, 2, 0, 0);
  scm_install_procedure ("auto-save",  tmg_auto_save, 0, 0, 0);
  scm_install_procedure ("get-style-menu",  tmg_get_style_menu, 0, 0, 0);
  scm_install_procedure ("get-add-package-menu",  tmg_get_add_package_menu, 0, 0, 0);
  scm_install_procedure ("get-remove-package-menu",  tmg_get_remove_package_menu, 0, 0, 0);
  scm_install_procedure ("style-clear-cache",  tmg_style_clear_cache, 0, 0, 0);
  scm_install_procedure ("set-script-status",  tmg_set_script_status, 1, 0, 0);
  scm_install_procedure ("set-printing-command",  tmg_set_printing_command, 1, 0, 0);
  scm_install_procedure ("set-printer-paper-type",  tmg_set_printer_paper_type, 1, 0, 0);
  scm_install_procedure ("get-printer-paper-type",  tmg_get_printer_paper_type, 0, 0, 0);
  scm_install_procedure ("set-printer-dpi",  tmg_set_printer_dpi, 1, 0, 0);
  scm_install_procedure ("set-default-shrinking-factor",  tmg_set_default_shrinking_factor, 1, 0, 0);
  scm_install_procedure ("get-default-shrinking-factor",  tmg_get_default_shrinking_factor, 0, 0, 0);
  scm_install_procedure ("get-nr-windows",  tmg_get_nr_windows, 0, 0, 0);
  scm_install_procedure ("image-gc",  tmg_image_gc, 0, 0, 0);
  scm_install_procedure ("inclusions-gc",  tmg_inclusions_gc, 0, 0, 0);
  scm_install_procedure ("update-all-path",  tmg_update_all_path, 1, 0, 0);
  scm_install_procedure ("update-all-buffers",  tmg_update_all_buffers, 0, 0, 0);
  scm_install_procedure ("set-message",  tmg_set_message, 2, 0, 0);
  scm_install_procedure ("set-message-temp",  tmg_set_message_temp, 3, 0, 0);
  scm_install_procedure ("recall-message",  tmg_recall_message, 0, 0, 0);
  scm_install_procedure ("yes?",  tmg_yesP, 1, 0, 0);
  scm_install_procedure ("quit-TeXmacs",  tmg_quit_TeXmacs, 0, 0, 0);
}
