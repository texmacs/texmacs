...
<\body>
  <expand|tmdoc-title|Notes pour les utilisateurs russes et ukrainiens>
  ...
  <\itemize>
    ...
    <apply|menu|Edit|Preferences|Language|Russian>. \ <apply|TeXmacs> charge
    ...
    document avec <apply|menu|Format|Language|Russian>.
  </itemize>
  ...
  de Linux et l'extension xkb est activée par défaut dans <with|font
  family|tt|XF86Config>. Avec l'extension xkb, les caractères sont écrits sur
  2 bytes et les caractères russes démarrent à 0x6??. Le clavier est
  configuré par <with|font family|tt|setxkbmap>. Lors de son lancement, X
  ...
  puis il envoie, s'il existe, le fichier utilisateur <with|font
  family|tt|~/.Xkbmap>. Le fichier <with|font family|tt|~/.Xkbmap> contient,
  en général, la ligne suivante :
  ...
  il envoie, s'il existe, le fichier utilisateur <with|font
  family|tt|~/.Xmodmap>. Vous pouvez définir la combinaison de touches pour
  changer de mode clavier et choisir un encodage russe (par exemple koi8-r)
  sur 1 byte lorsque vous êtes en mode russe. Il est plus facile, néanmoins,
  de télécharger le progiciel <with|font family|tt|xruskb> et de lancer :
  ...
  <apply|menu|Edit|Preferences|Keyboard|Cyrillic input method|Koi8-r>.

  Vous pouvez aussi utiliser l'encodage Windows cp1251 au lieu de koi8-r,
  bien que cela soit rare sous UNIX. Si vous utilisez <with|font
  family|tt|xrus jcuken-cp1251>, choisissez Cp1251 au lieu de Koi8-r.
  ...
  configurer de clavier. <apply|TeXmacs> fournit, dans ce cas, des méthodes
  de saisie en russe qui ne demandent aucune préparation. Ces méthodes n'ont
  d'effet que dans <apply|TeXmacs>.

  Le moyen le plus simple de saisir du russe sur un clavier standard US sans
  logiciel particulier est de choisir \ <apply|menu|Edit|Preferences|Keyboard|Cyrillic
  ...
  halign|c>|<table|<row|<cell|Raccourcis>|<cell|Pour>|<cell|Raccourcis>|<cell|Pour>>|<row|<cell|<expand|kbd-text|"
  ...
  fournies dans <apply|TeXmacs> et, si quelque chose ne vous convient pas,
  ...
  avec<key|majuscule temporaire>-nombre. <apply|TeXmacs> fournit un clavier
  yawerty légèrement modifié, car les touches <key|$>, <key|¿> et
  <key|<with|mode|math|\<backslash\>>>, qui servent par ailleurs dans
  <apply|TeXmacs>, ne sont pas redéfinies. Les caractères russes
  correspondant avec la combinaison de touches <key|majuscule
  temporaire>-nombre.
...
<\auxiliary>
  <\collection>
    <\associate|table>
      <tuple|normal|Saisie d'un texte cyrillique sur un clavier
      roman.|<pageref|gly-1>>
    </associate>
    <\associate|idx>
      ...
      family|<quote|ss>|Russe>>|<pageref|idx-1>>

      <tuple|<tuple|<with|font family|<quote|ss>|Document>|<with|font
      family|<quote|ss>|Langue>|<with|font
      family|<quote|ss>|Russe>>|<pageref|idx-2>>

      <tuple|<tuple|<with|font family|<quote|ss>|Format>|<with|font
      family|<quote|ss>|Langue>|<with|font
      family|<quote|ss>|Russe>>|<pageref|idx-3>>

      <tuple|<tuple|<with|font family|<quote|ss>|Éditer>|<with|font
      family|<quote|ss>|Préférences>|<with|font
      family|<quote|ss>|Clavier>|<with|font family|<quote|ss>|Cyrillique par
      le clavier>|<with|font family|<quote|ss>|translit>>|<pageref|idx-4>>
    </associate>
  </collection>
</auxiliary>
