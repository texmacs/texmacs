...
<\body>
  <apply|tmweb-current|Download|Sources><expand|tmweb-title|Installing
  <TeXmacs> under <name|Cygwin>|<apply|tmweb-download-links>>
  ...
  The last version of <name|Cygwin> can be installed from <hlink|<with|font
  family|tt|http://www.cygwin.com>|http://www.cygwin.com> as follows:
  ...
  <\enumerate>
    ...
    directory for the installation files (e.g. <with|font
    family|tt|C:\\Cygwin-installation>). It can take a long time (30 min) to
    download everything.

    <item>It is probably simplest to perform a complete installation. For
    each non selected package, you may select it by click on <with|font
    series|bold|Default> so that it becomes <with|font series|bold|Install>.
    However, you should <em|not> install the <with|font
    family|tt|ghostscript> package (you <em|should> install <with|font
    family|tt|ghostscript-base> and <with|font family|tt|ghostscript-x11>),
    by selecting <with|font series|bold|Skip> for this package.
    ...
    and all <name|Guile> packages (<with|font family|tt|guile>, <with|font
    family|tt|guile-devel>, <with|font family|tt|guile-doc>, <with|font
    family|tt|libguile12> and <with|font family|tt|libguile14>). You probably
    also want a decent window manager like <with|font family|tt|openbox> or
    <with|font family|tt|WindowMaker>.

    <item>Install.

    <item>Put <with|font family|tt|c:\\cygwin> at the start of the <with|font
    family|tt|PATH> environment variable and <with|font
    family|tt|127.0.0.1:0.0> for <with|font family|tt|DISPLAY> in the menu
    <with|font series|bold|Start-\<gtr\>Control
    ...
    <\itemize>
      ...
      <item>In the <name|Cygwin> window, launch the X server using <with|font
      family|tt|XWin&> and the window manager using <with|font
      family|tt|fvwm2&>.
  ...
  You may for instance save the distribution in <with|font
  family|tt|C:\\tmp>. The name of the file with the distribution is of the
  form <with|font family|tt|TeXmacs-[source version]-src.tar.gz>, where
  <with|font family|tt|[source version]> is the corresponding version.
  ...
  <\itemize>
    ...
    The file is a text-file that should be named <with|font
    family|tt|.xmodmap>, rename it to that if it's called something else
    (This won't work in Windows, use the <name|Cygwin> command <with|font
    family|tt|mv> instead, like
    ...
    <with|font family|tt|/usr/home>, i.e. <with|font
    family|tt|C:\\cygwin\\usr\\home>).
    ...
    and he had to set the <with|font family|tt|XbmModel> to <with|font
    family|tt|pc105> and the <with|font family|tt|XkbLayout> to <with|font
    family|tt|fr>.

    <item>If you want <TeXmacs> to correctly display images of most of the
    standard formats, then you should install the <with|font
    family|tt|netpbm> library:
    ...
    forget to put the following lines in your personal <with|font
    family|tt|$HOME/.bash_profile>:
...
<\auxiliary>
  <\collection>
    <\associate|toc>
      ...
      shape|<quote|small-caps>|Cygwin> with <with|font
      family|<quote|tt>|setup.exe>><value|toc-dots><pageref|toc-1><vspace|0.5fn>

      <vspace*|1fn><with|font series|<quote|bold>|math font
      series|<quote|bold>|2<space|2spc>Download the
      ...
      <vspace*|1fn><with|font series|<quote|bold>|math font
      series|<quote|bold>|3<space|2spc>Compile
      T<rsub|<space|-0.4spc><move|<resize|<with|index
      ...
      <vspace*|1fn><with|font series|<quote|bold>|math font
      series|<quote|bold>|4<space|2spc>Finishing
