...
;;
;; MODULE      : tmfs.scm
;; DESCRIPTION : TeXmacs file system
...
(request-handler (property-query . patterns)
  (property-match-several patterns '()))

(request-handler (property-solutions pattern)
  (with sols (property-match-several (list pattern) '())
    (map (cut bindings-substitute pattern <>) sols)))

(request-handler (property-add property)
  (property-change property #t))

(request-handler (property-remove pattern)
