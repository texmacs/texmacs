...
(tm-define (focus-variant-icons t)
  (append (opt #t
	       (list 'mini (lambda () #t)
		     (cons* '=>
			    (gui$balloon (tag-menu-name (tree-label t))
					 "Structured variant")
			    (variant-menu-items t))))
	  (opt (numbered-context? t)
	       ;; FIXME: itemize, enumerate, eqnarray*
	       (list (list 'check
			   (gui$balloon (gui$icon "tm_numbered.xpm")
					"Toggle numbering")
			   "v"
	  ...
	  (opt (toggle-context? t)
	       (list (list 'check
			   (gui$balloon (gui$icon "tm_unfold.xpm")
					"Fold / Unfold")
			   "v"
			   (lambda () (toggle-second-context? (focus-tree))))
		     (lambda () (toggle-toggle (focus-tree)))))
	  (opt #t
	       (list (gui$balloon (gui$icon "tm_focus_help.xpm")
				  "Describe tag")
		     (lambda () (set-message "Not yet implemented" ""))))
	  (opt #t
	       (list (gui$balloon (gui$icon "tm_focus_delete.xpm")
				  "Remove tag")
