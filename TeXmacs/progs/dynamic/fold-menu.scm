...
(menu-bind switch-menu
  (when (with t (tree-innermost dynamic-context?)
    ...
    ("Add branch before" (tree/switch-insert-at (focus-tree) :current))
    ("Add branch after" (tree/switch-insert-at (focus-tree) :var-next))
    ("Remove this branch" (tree/switch-remove-at (focus-tree) :current))
    ---
    (when (< 0 (tree/switch-index (focus-tree)))
      ("Switch to first" (dynamic-first)))
    (when (< 0 (tree/switch-index (focus-tree)))
      ("Switch to previous" (dynamic-previous)))
    (when (< (tree/switch-index (focus-tree)) (tree/switch-index (focus-tree) :last))
