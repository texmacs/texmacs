...
(define ($doc-all-symbols-buffer)
  ...
  (with sorted (sort (reverse (ahash-table->list indexed-commands)) sort-by-car)
    ($tmapidoc
      ($tmdoc-title (tr "All TeXmacs commands"))
      ($para 
...
(define ($doc-symbol-buffer ssym)
  ($tmapidoc
    ($tmdoc-title (tr "Documentation for %1" ssym))
    ($doc-explain-scm* ssym)))

(define ($doc-all-modules-buffer)
  ($tmapidoc ($tmdoc-title (tr "All TeXmacs modules"))
    ($doc-module-traverse '())))
...
(define ($doc-module-buffer smod)
  (with m (string->module smod)
    (if (is-real-module? m)
        ($tmapidoc
          ($tmdoc-title (tr "Documentation for %1" smod))
          `(document
            ,($doc-module-header m)
            ,($doc-module-exported m)))
        ($tmapidoc
          ($tmdoc-title (tr "Module family %1" smod))
          ($doc-module-traverse m)))))

(define ($query-not-implemented query)
 ($tmdoc 
   ($tmdoc-title (tr "Request unknown or not implemented"))
   ($para query)))
...
(tmfs-title-handler (apidoc query doc)
  (let* ((type (query-ref query "type"))
         (what (query-ref query "what")))
    (cond ((== type "symbol")
           (if (== what "") (tr "Help - Browse all symbols")
               (tr "Help - Documentation for " what)))
          ((== type "module")
           (if (== what "") (tr "Help - Browse all modules")
               (tr "Help - Documentation for module " what)))
          (else (tr "Help - Unknow request")))))
