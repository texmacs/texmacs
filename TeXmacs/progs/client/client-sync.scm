...
(tm-define (db-change-list uid t types)
...
(tm-define (db-change-status local-l remote-l)
  (let* ((local-t (db-change-table local-l))
    ...
    (for (name names)
      (let* ((local-pair (ahash-ref local-t name))
             ...
             (local-id (car local-pair))
             (remote-id (car remote-pair))
             (local-val (cadr local-pair))
             (remote-val (cadr remote-pair)))
