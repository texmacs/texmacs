...
def write_sample (path, table_path, syms, cov, per_row=3):
    ...
    w ("")
    w ("  Every symbol below is named in")
    w ("  <verbatim|tmuniversaltounicode-extra.scm>, classed in")
    w ("  <verbatim|std-symbols.scm> and exported by")
    w ("  <verbatim|latex-symbol-drd.scm> under the name unicode-math gives")
    w ("  it. A name the tables fail to serve shows as a box or an error")
    w ("  rather than a glyph, so this document tests them as well.")
    w ("")
    ...
    w ("")
    w ("  Each of these symbols has a class in <verbatim|std-symbols.scm>, so")
    w ("  it carries the spacing of its kind. Next to an older symbol, a")
    w ("  binary operator, two relations, an ordinary symbol and a product:")
    w ("")
